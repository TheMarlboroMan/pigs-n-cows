...
{
	return this->volcado(renderer, p_camara.acc_caja(), p_camara.acc_caja_pos(), p_camara.acc_desp_x(), p_camara.acc_desp_y());
}

//Explícitos, con pantalla...
bool Representacion::volcar(const Pantalla& p_pantalla, const Camara& p_camara)
{
	return this->volcado(p_pantalla.acc_renderer(), p_camara.acc_caja(), p_camara.acc_caja_pos(), p_camara.acc_desp_x(), p_camara.acc_desp_y());
}
...
bool Representacion::volcar(SDL_Renderer * renderer, const SDL_Rect& p_vista, const SDL_Rect& p_posicion, int desp_x, int desp_y)
{
	return this->volcado(renderer, p_vista, p_posicion, desp_x, desp_y);
}
